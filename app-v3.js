...
import { ancestryName, birthSurname, currentSurname, recordName } from './person-name-v1.js';
...
function canonicalName(person) {
  return ancestryName(person);
}

function firstName(person) {
  return ancestryName(person, { shortGiven: true });
}
...
function populateCentreSelect() {
  ...
  [...state.people].sort((a, b) => recordName(a).localeCompare(recordName(b))).forEach((item) => {
    const option = document.createElement('option');
    option.value = item.id;
    option.textContent = `${recordName(item)}${years(item) ? ` (${years(item)})` : ''}`;
...
function renderDetails(item) {
  ...
  ui.personName.textContent = recordName(item);
  const rows = [];
  if (preferredName(item)) rows.push(['Known as', preferredName(item)]);
  const birth = birthSurname(item);
  const current = currentSurname(item);
  if (item.birth_surname) rows.push(['Birth surname', birth]);
  if (item.current_surname && current.toLowerCase() !== birth.toLowerCase()) rows.push(['Married/current surname', current]);
  ...
  partnerEdgesOf(item.id).forEach(({ person: partner, relationship }) => {
    ...
    rows.push([label, `${recordName(partner)}${relationship.date_note ? ` - ${relationship.date_note}` : ''}`]);
  });
...
async function renderMapMode(centre, partner, token) {
  try {
    mapViewModulePromise ||= import('./map-view-v1.js?v=2');
...
function renderTree() {
  ...
  if (state.mode === 'map') {
    ...
    if (ui.viewSummary) ui.viewSummary.textContent = 'Click a pin to open the people, events, dates, place wording and evidence behind that family location. Routes appear only when movement has been separately evidenced.';
