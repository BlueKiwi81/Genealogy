import './family-viewer-access-bridge-v1.js';
...
// Force the current OTP input rules at runtime so stale HTML cannot leave a 6-digit cap behind.
otpCode.maxLength = 10;
otpCode.setAttribute('pattern', '[0-9]{6,10}');
otpCode.setAttribute('placeholder', '12345678');
...
function authCallbackError() {
  ...
  const candidates = [
    { source: 'hash', params: hash },
    { source: 'query', params: query },
  ];

  for (const candidate of candidates) {
    const error = candidate.params.get('error');
    const code = candidate.params.get('error_code');
    const description = candidate.params.get('error_description');
    if (error || code || description) return { source: candidate.source, error, code, description };
  }
  return null;
}

function clearAuthCallbackError(source) {
...
async function handleAuthCallbackError() {
  const failure = authCallbackError();
  if (!failure) return;

  // Supabase can leave an expired magic-link error in the address bar even when an
  // existing session is still valid. Clear the stale callback first, then only show
  // a warning when the visitor genuinely needs to sign in again.
  clearAuthCallbackError(failure.source);
  const { data: { session } } = await supabase.auth.getSession();
  if (session) return;

  const code = failure.code || failure.error || '';
  const description = String(failure.description || '').toLowerCase();
  if (code === 'otp_expired' || description.includes('expired') || description.includes('invalid')) {
    setMessage(authMessage, 'That email link has expired. Request a fresh sign-in code below and use the newest email you receive.', 'error');
    return;
  }
  setMessage(authMessage, failure.description || 'The email sign-in link could not be used. Request a fresh code below.', 'error');
}
...
void handleAuthCallbackError().catch(() => {
  // URL cleanup is best-effort and should never block ordinary sign-in.
});
