...
import { ancestryName, recordName, surnameSearchText } from '../person-name-v1.js';
...
const state = {
  ...
  access: [], tree: [], contributions: [], queue: [], filter: 'all', channels: [], objectUrls: [], evidenceFiles: new Map(), refreshTimer: null,
};

const esc = (value) => String(value ?? '').replace(/[&<>"']/g, (c) => ({ '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;' })[c]);
const norm = (value) => String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
const canonicalName = (p) => ancestryName(p, { unknown: 'Unknown person' });
const displayName = (p) => recordName({ ...p, given_names: p?.preferred_name?.trim() || p?.given_names?.trim() }, { unknown: 'Unknown person' });
...
function isVoiceContribution(item){ return item?.payload?.voice_recording || (Array.isArray(item?.payload?.categories) && item.payload.categories.includes('voice_note')); }
...
function matchScore(request, person){
  const surnames=[person.surname,person.birth_surname,person.current_surname].filter(Boolean);
  let score=0; if(request.birth_date && person.birth_date===request.birth_date) score+=8; if(request.last_name && surnames.some((surname)=>norm(surname)===norm(request.last_name))) score+=4;
...
async function loadReferenceData(){
  const [peopleRes,usersRes]=await Promise.all([
    supabase.from('people').select('id,given_names,preferred_name,surname,birth_surname,current_surname,birth_date,death_date,birth_place,death_place,occupation_summary,residence_summary,final_rest_type,final_rest_place,source_status').order('surname'),
...
function queueTitle(entry){
  ...
  if(entry.kind==='tree_change'){
    const target=targetName(item.target_person_id); if(item.change_type==='add_relative'){const r=item.payload?.relative||{};return `Add ${recordName(r,{unknown:'relative'})} to ${target}`;}
  ...
  if(isVoiceContribution(item)) return `Voice story about ${targetName(item.target_person_id)}`;
...
function queuePill(entry){ return entry.kind==='access_request'?'Access':entry.kind==='tree_change'?(entry.item.status==='conflict'?'Conflict':'Tree'):(isVoiceContribution(entry.item)?'Voice':entry.item.contribution_type==='story'?'Story':entry.item.contribution_type==='source'?'File':'Info'); }
...
function cleanupObjectUrls(){ state.objectUrls.forEach((u)=>URL.revokeObjectURL(u)); state.objectUrls=[]; state.evidenceFiles.clear(); }
...
function treeRows(change){
  const rows=[]; if(change.change_type==='add_relative'){const r=change.payload?.relative||{}; rows.push(['Relationship',change.payload?.role||'relative'],['Name',recordName(r,{unknown:'Not supplied'})]); if(r.birth_surname)rows.push(['Birth / maiden surname',r.birth_surname]); if(r.current_surname)rows.push(['Married / current surname',r.current_surname]); [['preferred_name','Known as'],['life_status','Life status'],['birth_date','Birth'],['birth_place','Birth place'],['residence_summary','Where they lived'],['death_date','Death'],['death_place','Death place'],['occupation_summary','Occupation'],['military_service_summary','Military / service'],['narrative_summary','Family note']].forEach(([key,label])=>{if(r[key])rows.push([label,human(r[key])]);}); if(r.final_rest_type||r.final_rest_place)rows.push(['Final resting place',[human(r.final_rest_type),r.final_rest_place].filter(Boolean).join(' — ')]); rows.push(...historyRows(r.historical_context));}
...
async function evidenceHtml(items){
  const parts=[]; for(const item of items){ let media=''; try{const {data,error}=await supabase.storage.from('family-evidence').download(item.storage_path); if(error)throw error; const url=URL.createObjectURL(data); state.objectUrls.push(url); state.evidenceFiles.set(item.id,{blob:data,url,filename:item.original_filename||'family-recording',type:data.type||''}); const type=data.type||''; const filename=(item.original_filename||'').toLowerCase(); if(type.startsWith('audio/')||/\.(m4a|mp3|webm|ogg|wav|aac|mp4)$/.test(filename)) media=`<audio class="evidence-audio" controls preload="metadata" src="${esc(url)}"></audio><div class="evidence-file-actions"><a class="button secondary document-link" href="${esc(url)}" download="${esc(item.original_filename||'family-recording')}">Download recording</a><button class="button secondary evidence-share" type="button" data-share-evidence="${esc(item.id)}">Share recording</button></div>`; else if(type.startsWith('image/')||/\.(jpg|jpeg|png|webp|heic|heif|tif|tiff)$/.test(filename)) media=`<img src="${esc(url)}" alt="${esc(item.title||item.original_filename||'Family photograph')}" />`; else media=`<a class="button secondary document-link" href="${esc(url)}" target="_blank" rel="noopener">Open document</a>`; }catch(error){media=`<p class="message error">${esc(error.message||'Unable to load file')}</p>`;}
...
async function shareEvidence(id){
  const file=state.evidenceFiles.get(id);if(!file)return toast('The recording is no longer open.');
  const shareFile=new File([file.blob],file.filename,{type:file.type||file.blob.type||'audio/mp4'});
  if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[shareFile]}))){
    try{await navigator.share({title:'Family voice recollection',text:'Private family recollection for transcription or review.',files:[shareFile]});return;}catch(error){if(error?.name==='AbortError')return;}
  }
  const link=document.createElement('a');link.href=file.url;link.download=file.filename;link.click();toast('Sharing is unavailable here, so the recording was downloaded instead.');
}
async function openContribution(item){
  const voice=isVoiceContribution(item);const transcriptValue=voice?'':(item.narrative_text||'');
  showDetail(voice?'Voice recollection':item.contribution_type==='story'?'Family story':item.contribution_type==='source'?'Photo / source':'Family contribution',targetName(item.target_person_id),`<p class="detail-meta">${esc(userName(item.submitted_by))} · ${esc(formatTime(item.created_at))}</p><div class="story-text">${esc(item.narrative_text||'No written description supplied.')}</div><div id="evidenceArea"></div><label>${voice?'Transcript or edited family narrative (optional)':'Approved wording / review note'}<textarea id="contributionEdit" rows="7"${voice?' placeholder="Paste a transcript or write the reviewed narrative here. Leave blank to approve the original recording as oral evidence only."':''}>${esc(transcriptValue)}</textarea></label><div class="sheet-actions"><button id="approveContributionButton" class="button primary" type="button">${voice?'Approve recording':'Approve'}</button><button id="rejectContributionButton" class="button danger" type="button">Reject</button></div>`);
  if(evidenceRefs(item.payload).length){ try{const evidence=await evidenceFor(item); const html=await evidenceHtml(evidence); const area=document.getElementById('evidenceArea'); if(area){area.innerHTML=`<div class="evidence-grid">${html||'<div class="queue-empty">No attached file found.</div>'}</div>`;area.querySelectorAll('[data-share-evidence]').forEach(button=>button.addEventListener('click',()=>shareEvidence(button.dataset.shareEvidence)));}}catch(error){document.getElementById('evidenceArea').innerHTML=`<p class="message error">${esc(error.message)}</p>`;} }
...
async function approveContribution(item){
  ...
  const voice=isVoiceContribution(item);
  if(item.contribution_type==='story'&&item.target_person_id&&!voice){ if(!edited)return toast('Approved story text cannot be blank.'); const {error}=await supabase.from('narratives').insert({person_id:item.target_person_id,title:'Family recollection',original_language:item.original_language||'en',original_text:item.narrative_text||'',edited_text:edited,narrative_status:'approved',source_status:'family_supplied'});if(error)return toast(error.message); }
  if(voice&&edited&&item.target_person_id){const {error}=await supabase.from('narratives').insert({person_id:item.target_person_id,title:'Oral recollection',original_language:item.original_language||'en',original_text:item.narrative_text||'Original audio preserved in the evidence record.',edited_text:edited,narrative_status:'approved',source_status:'family_supplied'});if(error)return toast(error.message);}
  const refs=evidenceRefs(item.payload); if(refs.length){const {error}=await supabase.from('evidence_items').update({review_status:'approved',privacy_review_status:'cleared',reviewed_by:state.session.user.id,reviewed_at:now}).in('id',refs.map((r)=>r.id));if(error)return toast(error.message);}
  const note=voice?(edited?'Approved as oral recollection with a reviewed transcript or narrative.':'Approved as oral recollection evidence only; no transcript added.'):item.contribution_type==='story'?'Approved as a family narrative.':edited||'Approved for structured incorporation into the canonical tree.'; const {error}=await supabase.from('contributions').update({status:'approved',review_note:note,reviewed_by:state.session.user.id,reviewed_at:now}).eq('id',item.id);if(error)return toast(error.message); closeDetail();toast(voice?'Voice recollection approved.':'Contribution approved.');await refreshAll();
}
async function rejectContribution(item){ const now=new Date().toISOString(); const refs=evidenceRefs(item.payload);if(refs.length){const {error}=await supabase.from('evidence_items').update({review_status:'rejected',reviewed_by:state.session.user.id,reviewed_at:now}).in('id',refs.map((r)=>r.id));if(error)return toast(error.message);} const {error}=await supabase.from('contributions').update({status:'rejected',review_note:'Rejected by family editor.',reviewed_by:state.session.user.id,reviewed_at:now}).eq('id',item.id);if(error)return toast(error.message);closeDetail();toast('Contribution rejected.');await refreshAll(); }

function renderPeople(){
  const q=norm(ui.peopleSearch.value); const list=state.people.filter((p)=>!q||norm(`${p.given_names} ${p.preferred_name||''} ${surnameSearchText(p)}`).includes(q)).slice(0,80);
...
function openPerson(id){ const p=state.peopleById.get(id); if(!p)return; const rows=[['Ancestry name',canonicalName(p)],p.birth_surname?['Birth surname',p.birth_surname]:null,p.current_surname?['Married / current surname',p.current_surname]:null,p.preferred_name?['Known as',p.preferred_name]:null,p.birth_date?['Born',`${formatDate(p.birth_date)}${p.birth_place?` — ${p.birth_place}`:''}`]:null,p.death_date?['Died',`${formatDate(p.death_date)}${p.death_place?` — ${p.death_place}`:''}`]:null,p.residence_summary?['Lived',p.residence_summary]:null,p.occupation_summary?['Occupation',p.occupation_summary]:null,(p.final_rest_type||p.final_rest_place)?['Final resting place',[human(p.final_rest_type),p.final_rest_place].filter(Boolean).join(' — ')]:null,['Source status',human(p.source_status||'unresolved')]].filter(Boolean); showDetail('Person record',displayName(p),`<div class="detail-grid">${rows.map(([l,v])=>`<div class="detail-row"><strong>${esc(l)}</strong><span>${esc(v)}</span></div>`).join('')}</div><div class="sheet-actions single"><button id="personPhotoShortcut" class="button primary" type="button">Add photograph</button></div>`); document.getElementById('personPhotoShortcut').addEventListener('click',()=>{closeDetail();ui.photoPerson.value=p.id;switchTab('photos');}); }
