...
function syncWorkspaceBoundary(workspace, treePanel, sideColumn) {
  ...
  if (!window.matchMedia('(min-width: 1041px)').matches) {
    if (sideColumn.style.getPropertyValue('--tree-workspace-height')) sideColumn.style.removeProperty('--tree-workspace-height');
    return;
  }
  const height = Math.max(0, Math.ceil(treePanel.getBoundingClientRect().height));
  if (!height) return;
  const next = `${height}px`;
  if (sideColumn.style.getPropertyValue('--tree-workspace-height') !== next) {
    sideColumn.style.setProperty('--tree-workspace-height', next);
  }
}

function observeTreePanel(treePanel, workspace, sideColumn) {
  ...
  treeResizeObserver = new ResizeObserver(() => syncWorkspaceBoundary(workspace, treePanel, sideColumn));
...
function enforceLayout() {
  ...
  // DOM ordering is owned by workspace-layout-v2. This guard only keeps the
  // selected-person pane bounded to the tree height; it no longer moves tools.
  if (personPanel) personPanel.classList.add('selected-person-sticky');
  syncWorkspaceBoundary(workspace, treePanel, sideColumn);
  observeTreePanel(treePanel, workspace, sideColumn);
}
...
function settleLayout() {
  scheduleArrange();
  window.setTimeout(scheduleArrange, 120);
  window.setTimeout(scheduleArrange, 420);
}
