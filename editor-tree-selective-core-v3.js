...
function installStyles(){
  if(document.getElementById('selectiveCoreV3Styles')) return;
  const s=document.createElement('style');
  s.id='selectiveCoreV3Styles';
  s.textContent=`
    .selective-core-v3{margin:12px 0;padding:12px;border:2px solid rgba(91,72,55,.22);border-radius:12px;background:#fffdf8}
    .selective-core-v3 h4{margin:0 0 4px;font-size:13px}.selective-core-v3 p{margin:4px 0 9px;font-size:10px;line-height:1.45}
    .selective-core-v3-badge{display:inline-block;padding:4px 8px;border-radius:999px;background:#eef3ea;color:#4d704f;font-size:9px;font-weight:700;margin-bottom:7px}
    .selective-core-v3-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center;padding:9px;margin:7px 0;border:1px solid rgba(91,72,55,.16);border-radius:9px;background:#fff}
    .selective-core-v3-row.blocked{background:#fff3f0;border-color:rgba(140,65,65,.34)}
    .selective-core-v3-copy{display:grid;gap:2px;min-width:0}.selective-core-v3-copy strong{font-size:9px;text-transform:uppercase;color:#645548}.selective-core-v3-copy small,.selective-core-v3-copy b{font-size:9.5px;white-space:pre-wrap}
    .selective-core-v3-controls{display:flex;gap:5px}.selective-core-v3-controls label{display:inline-flex!important;align-items:center!important;gap:4px!important;padding:5px 7px;border:1px solid rgba(91,72,55,.2);border-radius:999px;font-size:9px!important;font-weight:700!important}.selective-core-v3-controls input{width:auto!important;margin:0!important}
    .selective-core-v3 select,.selective-core-v3 textarea{width:100%;box-sizing:border-box;border:1px solid rgba(91,72,55,.2);border-radius:8px;padding:7px;background:#fff}.selective-core-v3 label.selective-block{display:grid!important;gap:4px!important;margin:8px 0;font-size:9.5px!important;font-weight:700}
    .selective-core-v3-status{font-size:9.5px!important}.selective-core-v3-status.error{color:#8a3e36}.selective-core-v3-status.success{color:#466648}
  `;
  document.head.appendChild(s);
}

function mount(card){
  let box=card.querySelector('.selective-core-v3');
  if(box) return box;
  box=document.createElement('section');
  box.className='selective-core-v3';
  box.innerHTML='<span class="selective-core-v3-badge">Field review loading…</span><p>Loading the pending proposal and current canonical record.</p>';
...
function radioRow(key,current,proposed,accept=false){
  const name=`sc3-${key}-${Math.random().toString(36).slice(2)}`;
  return `<div class="selective-core-v3-row" data-sc3-field="${esc(key)}"><div class="selective-core-v3-copy"><strong>${esc(LABELS[key]||key)}</strong><small>Current: ${esc(text(current))}</small><b>Proposed: ${esc(text(proposed))}</b></div><div class="selective-core-v3-controls"><label><input type="radio" name="${name}" value="accept" ${accept?'checked':''}>Accept</label><label><input type="radio" name="${name}" value="reject" ${accept?'':'checked'}>Reject</label></div></div>`;
}
...
async function loadContext(id){
  ...
  return {change,target:targetR.data,people:peopleR.data||[],rels:relsR.data||[]};
}

function bindApply(box,ctx){
  box.querySelector('[data-sc3-apply]')?.addEventListener('click',async(e)=>{
    const btn=e.currentTarget, status=box.querySelector('.selective-core-v3-status');
    const fields=[...box.querySelectorAll('[data-sc3-field]')].filter(r=>r.querySelector('input[value="accept"]')?.checked).map(r=>r.dataset.sc3Field);
    const rel=Boolean(box.querySelector('[data-sc3-rel] input[value="accept"]:checked'));
    const existing=box.querySelector('[data-sc3-existing]')?.value||null;
    if(ctx.change.change_type==='add_relative'&&!existing){status.textContent='Choose the existing person first.';status.className='selective-core-v3-status error';return;}
    if(!fields.length&&!rel){status.textContent='Nothing is marked Accept. Use “Reject and restore” to reject the whole proposal.';status.className='selective-core-v3-status error';return;}
    btn.disabled=true; btn.textContent='Applying…';
    try{
      const {data,error}=await supabase.rpc('approve_tree_change_selection',{p_change_set_id:ctx.change.id,p_selected_fields:fields,p_existing_person_id:existing,p_apply_relationship:rel,p_note:box.querySelector('[data-sc3-note]')?.value.trim()||null});
      ...
      status.className='selective-core-v3-status success';
      ...
      status.className='selective-core-v3-status error';
...
function renderEdit(box,ctx){
  ...
  box.innerHTML=`<span class="selective-core-v3-badge">Field-by-field review ready</span><h4>Accept or reject each proposed change</h4><p>The server will verify that an Intelligent Review exists before applying your decisions.</p>${rows.map(k=>radioRow(k,current[k]??null,proposed[k],current[k]==null||current[k]==='')).join('')||'<p>No differing fields remain.</p>'}<label class="selective-block">Editor note<textarea rows="2" data-sc3-note></textarea></label><button type="button" class="button primary" data-sc3-apply>Apply accepted / rejected decisions</button><p class="selective-core-v3-status"></p>`;
  bindApply(box,ctx);
}
...
function renderRelative(box,ctx){
  ...
  box.innerHTML=`<span class="selective-core-v3-badge">Field-by-field review ready</span><h4>Accept or reject each proposed change</h4><p>The server will verify that an Intelligent Review exists before applying your decisions.</p><label class="selective-block">Existing person<select data-sc3-existing><option value="">Choose existing person…</option>${opts}</select></label><div data-sc3-rows></div><label class="selective-block">Editor note<textarea rows="2" data-sc3-note></textarea></label><button type="button" class="button primary" data-sc3-apply>Apply accepted / rejected decisions</button><p class="selective-core-v3-status"></p>`;
  const select=box.querySelector('[data-sc3-existing]'), host=box.querySelector('[data-sc3-rows]');
  const refresh=()=>{
    ...
    const name=`sc3-rel-${Math.random().toString(36).slice(2)}`;
    host.innerHTML=`${keys.map(k=>radioRow(k,existing[k]??null,proposed[k],existing[k]==null||existing[k]==='')).join('')||'<p>No differing person fields remain.</p>'}<div class="selective-core-v3-row ${ri.blocked?'blocked':''}" data-sc3-rel><div class="selective-core-v3-copy"><strong>Relationship</strong><b>${esc(ri.msg)}</b></div><div class="selective-core-v3-controls"><label><input type="radio" name="${name}" value="accept" ${ri.blocked?'disabled':''}>Accept</label><label><input type="radio" name="${name}" value="reject" checked>Reject</label></div></div>`;
  };
...
async function ensure(card){
  ...
  try{
    ...
    const box=mount(card);
    box.innerHTML=`<span class="selective-core-v3-badge">Field review error</span><p class="selective-core-v3-status error">${esc(error?.message||String(error))}</p>`;
  }
}
...
setInterval(scanSelectiveReview,5000);
...
document.getElementById('refreshEditor')?.addEventListener('click',()=>setTimeout(scanSelectiveReview,350));
