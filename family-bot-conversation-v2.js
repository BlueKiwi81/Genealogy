...
function languageOf(text) {
  ...
  const afHits = (value.match(/\b(wat|wie|waar|wanneer|waarom|hoekom|gebore|oorlede|ouers|vader|moeder|bewys|navors|onseker|onthou|vertel|familie)\b/g) || []).length;
  const enHits = (value.match(/\b(what|who|where|when|why|born|died|parents|father|mother|evidence|research|uncertain|remember|tell|family)\b/g) || []).length;
...
function installStyles() {
  if ($('familyBotConversationV2Styles')) return;
  const style = document.createElement('style');
  style.id = 'familyBotConversationV2Styles';
  style.textContent = `
    .k3v2-panel{position:fixed;left:18px;bottom:88px;z-index:1199;width:min(430px,calc(100vw - 36px));height:min(620px,calc(100dvh - 118px));display:none;grid-template-rows:auto auto 1fr auto;background:#fffaf3;border:1px solid #cdbdac;border-radius:18px;box-shadow:0 22px 55px rgba(46,36,28,.26);overflow:hidden;font-family:Arial,sans-serif}.k3v2-panel.open{display:grid}
    .k3v2-head{display:flex;justify-content:space-between;gap:12px;padding:14px 15px;background:#4a3b2f;color:white}.k3v2-head strong{font-size:15px}.k3v2-head small{display:block;margin-top:2px;color:#e8ddd1;font-size:10px}.k3v2-close{border:0;background:none;color:white;font:700 20px/1 Arial;cursor:pointer}
    .k3v2-context{padding:9px 13px;background:#f8f1e8;border-bottom:1px solid #e5dbcf;color:#67594d;font:700 10px/1.35 Arial}.k3v2-context span{font-weight:500}.k3v2-messages{overflow:auto;padding:14px;display:flex;flex-direction:column;gap:11px}.k3v2-msg{max-width:92%;padding:10px 11px;border-radius:14px;font:12px/1.5 Arial;white-space:pre-wrap}.k3v2-msg.user{align-self:flex-end;background:#4a3b2f;color:white;border-bottom-right-radius:4px}.k3v2-msg.bot{align-self:flex-start;background:#f3ece3;color:#40362e;border-bottom-left-radius:4px}.k3v2-msg.system{align-self:center;max-width:100%;background:#fff5df;border:1px solid #ead6ad;color:#6c5737;text-align:center;font-size:11px}.k3v2-msg.loading{font-style:italic;color:#796a5d}
    ...
    .k3v2-form{display:grid;grid-template-columns:1fr auto;gap:8px;padding:10px;background:white;border-top:1px solid #e3d8cc}.k3v2-input{resize:none;min-height:42px;max-height:118px;border:1px solid #cfc0b0;border-radius:12px;padding:9px 10px;font:12px/1.4 Arial}.k3v2-send{height:42px;min-width:58px;border:0;border-radius:12px;background:#4a3b2f;color:white;font:800 10px Arial;cursor:pointer}
    @media(max-width:600px){.k3v2-panel{left:8px;bottom:76px;width:calc(100vw - 16px);height:min(650px,calc(100dvh - 92px))}}
  `;
...
function buildUi() {
  ...
  panel.innerHTML = `
    <header class="k3v2-head"><div><strong>K-3 Family Droid</strong><small>${t('Ask, remember, contribute', 'Vra, onthou, dra by')}</small></div><button id="familyBotV2Close" class="k3v2-close" type="button">x</button></header>
    <div id="familyBotV2Context" class="k3v2-context"><strong>${t('Current focus:', 'Huidige fokus:')}</strong> <span></span></div>
    <div id="familyBotV2Messages" class="k3v2-messages" aria-live="polite"></div>
    <form id="familyBotV2Form" class="k3v2-form"><textarea id="familyBotV2Input" class="k3v2-input" rows="2" maxlength="6000"></textarea><button id="familyBotV2Send" class="k3v2-send" type="submit">${t('Send', 'Stuur')}</button></form>`;
  ...
  $('familyBotV2Form')?.addEventListener('submit', (event) => {
    ...
    const text = $('familyBotV2Input')?.value.trim();
    if (!text || busy) return;
    $('familyBotV2Input').value = '';
    if (capture) captureMemory(text); else askQuestion(text);
  });
  ...
  document.addEventListener('genealogy:language-changed', () => { updateContext(); setPlaceholder(); });
  updateContext();
  setPlaceholder();
}
function openPanel() {
...
function closePanel() { $('familyBotV2Panel')?.classList.remove('open'); }

function renderAmbiguity(data, question) {
  const card = addMessage('bot', data.message || t('Which person did you mean?', 'Watter persoon bedoel jy?'));
  if (!card) return;
  ...
  (data.candidates || []).forEach((candidate) => actions.appendChild(action(
    ...
    () => askQuestion(question, candidate.id, true)
  )));
...
function renderAnswer(data, question) {
  ...
  const actions = document.createElement('div');
  actions.className = 'k3v2-actions';
  actions.appendChild(action(t('Share what you know', 'Deel wat jy weet'), () => beginContribution(data, question), true));
  if (answer.offer_research && answer.research_question) {
    actions.appendChild(action(t('Research this question', 'Navors hierdie vraag'), () => handoffResearch(data.target?.id, answer.research_question)));
  }
  card.appendChild(actions);
...
async function askQuestion(question, personId = '', silent = false) {
  if (busy) return;
  capture = null;
  setPlaceholder();
  if (!silent) {
  ...
  try {
    const { data, error } = await supabase.functions.invoke('genealogy-family-bot-v2', {
      body: { mode: 'ask', question, language: languageOf(question), person_id: personId || undefined, selected_person_id: centreSelect?.value || undefined, conversation: history.slice(-6) },
    });
    loading?.remove();
    ...
    if (data.status === 'needs_person' || data.status === 'privacy_limited') return addMessage('bot', data.message);
    if (data.status === 'answered') {
      renderAnswer(data, question);
      history.push({ role: 'assistant', text: data.answer?.answer || '' });
      return;
    }
    addMessage('bot', t('I could not turn that into an archive answer yet.', 'Ek kon dit nog nie in n argiefantwoord omskep nie.'));
  } catch (error) {
    loading?.remove();
...
function beginContribution(data, question) {
  capture = { targetId: data.target?.id || centreSelect?.value || '', targetName: data.target?.name || currentName() || t('this person', 'hierdie persoon'), question, memories: [] };
  setPlaceholder();
  addMessage('bot', t(
...
async function captureMemory(memory) {
  ...
  setBusy(true);
  const loading = addMessage('bot loading', t('K-3 is preparing a contribution draft...', 'K-3 berei n bydraekonsep voor...'));
  const raw = capture.memories.join('\n\n');
  try {
    const { data, error } = await supabase.functions.invoke('genealogy-family-bot-v2', {
      body: { mode: 'capture_contribution', person_id: capture.targetId || undefined, selected_person_id: centreSelect?.value || undefined, question: capture.question, memory: raw, language: languageOf(raw) },
    });
    loading?.remove();
...
function renderDraft(draft, fallback = false) {
  ...
  summary.innerHTML = `<h4>${t('What I understood', 'Wat ek verstaan het')}</h4>`;
  ...
  if (fallback) {
    ...
    note.textContent = t('I could not safely extract clues, but your own words are preserved and can still be submitted.', 'Ek kon nie die leidrade veilig uithaal nie, maar jou eie woorde is behou en kan steeds ingedien word.');
  ...
  if (Array.isArray(draft.clues) && draft.clues.length) {
    ...
    heading.textContent = t('Possible leads - not established facts', 'Moontlike leidrade - nie vasgestelde feite nie');
    section.appendChild(heading);
    const list = document.createElement('ul');
    draft.clues.slice(0, 8).forEach((clue) => { const li = document.createElement('li'); li.textContent = [clue.kind, clue.detail].filter(Boolean).join(': '); list.appendChild(li); });
  ...
  const actions = document.createElement('div');
  actions.className = 'k3v2-actions';
  actions.appendChild(action(t('Add to contribution form', 'Voeg by bydraevorm'), () => handoffContribution(draft), true));
  actions.appendChild(action(t('Tell K-3 more', 'Vertel K-3 meer'), () => { setPlaceholder(); addMessage('bot', t('Go ahead - add anything else you remember.', 'Gaan voort - voeg enigiets anders by wat jy onthou.')); $('familyBotV2Input')?.focus(); }));
  card.appendChild(actions);
  host.appendChild(card);
  host.scrollTop = host.scrollHeight;
}
...
async function handoffContribution(draft) {
  ...
  categoryInputs.forEach((box) => { box.checked = categories.has(box.value); box.dispatchEvent(new Event('change', { bubbles: true })); });
  if (!categoryInputs.length && $('contributionType')) $('contributionType').value = [...categories][0] || 'story';
  const lang = languageOf(draft.raw_memory || capture.memories.join('\n\n'));
  if ($('contributionLanguageSelect')) { $('contributionLanguageSelect').value = lang; $('contributionLanguageSelect').dataset.touched = '1'; }
  if ($('language')) $('language').value = lang;
  const clues = Array.isArray(draft.clues) ? draft.clues : [];
  const clueBlock = clues.length ? `\n\n${t('K-3 possible leads for review (not established facts):', 'K-3 moontlike leidrade vir hersiening (nie vasgestelde feite nie):')}\n${clues.slice(0,8).map((clue) => `- ${[clue.kind, clue.detail].filter(Boolean).join(': ')}`).join('\n')}` : '';
...
async function handoffResearch(targetId, question) {
  ...
  if (!area) return addMessage('system', t('I could not open the research assistant automatically.', 'Ek kon nie die navorsingsassistent outomaties oopmaak nie.'));
  area.value = question;
...
export async function openFamilyBot() { openPanel(); }
