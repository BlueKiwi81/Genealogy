...
function styles(){
  ...
  const s=document.createElement('style'); s.id='selectiveCoreStyles'; s.textContent=`
...
function mount(card){
  ...
  if(box) return box;
  box=document.createElement('section'); box.className='selective-core';
  ...
  const note=card.querySelector('.tree-change-note'); if(note) note.insertAdjacentElement('beforebegin',box); else card.appendChild(box);
  const approve=card.querySelector('[data-approve-tree-change]'); if(approve){approve.disabled=true;approve.textContent='Use field decisions below';}
  return box;
}

function score(relative,p){
  let n=0; const rg=norm(relative?.given_names), pg=norm(p?.given_names);
...
async function load(id){
  const {data:change,error:e1}=await supabase.from('tree_change_sets').select('id,target_person_id,change_type,payload,before_snapshot,status').eq('id',id).maybeSingle(); if(e1) throw e1;
  if(!change||change.status!=='pending'||!['edit_person','add_relative'].includes(change.change_type)) return null;
  const {data:reviews,error:e2}=await supabase.from('tree_change_ai_reviews').select('decision,confidence,summary,rationale,warnings,created_at').eq('change_set_id',id).order('created_at',{ascending:false}).limit(1); if(e2) throw e2;
  ...
  if(targetR.error)throw targetR.error;if(peopleR.error)throw peopleR.error;if(relsR.error)throw relsR.error;
...
function bindApply(box,ctx){
  box.querySelector('[data-sc-apply]')?.addEventListener('click',async(e)=>{
    ...
    status.textContent=`Applied ${data?.selected_fields?.length??fields.length} accepted field(s). Rejected items were left unchanged.`;status.className='selective-core-status success';
    document.dispatchEvent(new CustomEvent('genealogy:tree-suggestions-updated'));
  });
}

function renderEdit(box,ctx){
  const proposed=ctx.change.payload?.after||{}; const current=ctx.target||ctx.change.before_snapshot?.person||{};
...
function renderRelative(box,ctx){
  ...
  const refresh=()=>{const existing=ctx.people.find(p=>p.id===select.value);if(!existing){host.innerHTML='<p>Choose the existing person first.</p>';return;}const keys=FIELDS.filter(k=>Object.prototype.hasOwnProperty.call(proposed,k)&&proposed[k]!==null&&proposed[k]!==''&&!same(proposed[k],existing[k]??null));const ri=relationshipInfo(ctx.change,ctx.target,existing,ctx.rels);const name=`sc-rel-${Math.random().toString(36).slice(2)}`;host.innerHTML=`${keys.map(k=>radioRow(k,existing[k]??null,proposed[k],existing[k]==null||existing[k]==='')).join('')||'<p>No differing person fields remain.</p>'}<div class="selective-core-row ${ri.blocked?'blocked':''}" data-sc-rel><div class="selective-core-copy"><strong>Relationship</strong><b>${esc(ri.msg)}</b></div><div class="selective-core-controls"><label><input type="radio" name="${name}" value="accept" ${ri.blocked?'disabled':''}>Accept</label><label><input type="radio" name="${name}" value="reject" checked>Reject</label></div></div>`;};
  select.addEventListener('change',refresh);refresh();bindApply(box,ctx);
}

async function ensure(card){
  const id=card?.dataset?.treeChangeId;if(!id||busy.has(id))return;busy.add(id);const box=mount(card);
  try{const ctx=await load(id);if(!ctx){box.remove();return;}if(!ctx.review){box.innerHTML='<span class="selective-core-badge">Field review waiting</span><p>Run Intelligent Review for this proposal first. Whole-item approval is disabled for safety.</p>';return;}if(ctx.change.change_type==='edit_person')renderEdit(box,ctx);else renderRelative(box,ctx);}catch(err){box.innerHTML=`<span class="selective-core-badge">Field review error</span><p class="selective-core-status error">${esc(err?.message||String(err))}</p>`;}finally{busy.delete(id);}
}
function scan(){document.querySelectorAll('[data-tree-change-id]').forEach(c=>void ensure(c));}
function schedule(){if(timer)clearTimeout(timer);timer=setTimeout(()=>{timer=null;scan();},100);}
styles();scan();new MutationObserver(schedule).observe(document.body,{childList:true,subtree:true});document.addEventListener('genealogy:tree-suggestions-updated',schedule);
