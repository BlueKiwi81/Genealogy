...
const MAP_CSS = './map-view-v1.css?v=6';
...
function pinHtml(itemCount, indexes, branches, mode) {
  const background = markerBackground(indexes, branches);
  const label = itemCount > 1 ? String(itemCount) : mode === 'context' ? 'H' : '';
  if (mode === 'context') {
    return `<span class="map-context-pin" style="--map-pin-fill:${esc(background)}"><span>${esc(label)}</span></span>`;
  }
  return `<span class="map-place-pin${mode === 'mixed' ? ' is-mixed' : ''}" style="--map-pin-fill:${esc(background)}"><span>${esc(label)}</span></span>`;
}
...
function eventPopupRows(events, peopleById, eventBranches, branches) {
  return events.map((event) => {
    ...
    return `<article class="map-popup-event">
      ...
      <span class="map-popup-meta"><span>${esc(EVIDENCE_LABELS[evidenceGroup(event.source_status)])}</span><span>${esc(locationLabel(event))}</span><span>${esc(branchTextFor(eventBranches.get(event.id) || [], branches))}</span></span>
...
function contextPopupRows(contexts, peopleById, contextBranches, branches) {
  return contexts.map((context) => {
    ...
    return `<article class="map-popup-event map-popup-context">
      ...
      <span class="map-popup-meta"><span>${esc(EVIDENCE_LABELS[evidenceGroup(context.source_status)])}</span><span>${esc(locationLabel(context))}</span><span>${esc(linkLabel)}</span><span>${esc(branchTextFor(contextBranches.get(context.id) || [], branches))}</span></span>
...
function popupHtml(name, events, contexts, peopleById, eventBranches, contextBranches, branches) {
  ...
  const kicker = mode === 'mixed' ? 'Family place and historical context' : mode === 'context' ? 'Historical / research context' : 'Reviewed family place';
  const countParts = [];
  if (events.length) countParts.push(`${events.length} family event${events.length === 1 ? '' : 's'}`);
  if (contexts.length) countParts.push(`${contexts.length} context item${contexts.length === 1 ? '' : 's'}`);
  return `<div class="map-popup"><p class="map-popup-kicker">${esc(kicker)}</p><h3>${esc(name)}</h3><p class="map-popup-count">${esc(countParts.join(' + '))} at this mapped locality</p>${eventPopupRows(events, peopleById, eventBranches, branches)}${contextPopupRows(contexts, peopleById, contextBranches, branches)}</div>`;
}
...
export async function renderMapView({ centre, partner, people, relationships, palette }) {
  ...
  canvas.innerHTML = `<div class="map-view-shell">
    <section class="map-view-controls" aria-label="Map filters">
      <div><p class="map-filter-title">Dynamic family branches</p><div id="mapBranchLegend" class="map-branch-legend"></div></div>
      <div class="map-filter-stack">
        <div><p class="map-filter-title">Map content</p><div class="map-content-filters"><label><input type="checkbox" data-map-content="event" checked> Family events</label><label><input type="checkbox" data-map-content="context" checked> Historical / research context</label></div><p class="map-filter-help">Context is stored separately from personal life events, so a wartime site or research place can be shown without claiming that an ancestor was personally there.</p></div>
    ...
    <div id="historicalMap" class="historical-map" role="region" aria-label="Historical family locations"></div>
  ...
  function renderLayers() {
    ...
    function addRecord(record, kind) {
      ...
      if (!coordinates) {
        unresolved.push({ record, kind });
        return;
      }
      const key = `${coordinates[0].toFixed(5)},${coordinates[1].toFixed(5)}`;
      const group = groups.get(key) || { coordinates, events: [], contexts: [], name: record.place_ref?.canonical_name || record.place || 'Recorded place' };
      if (kind === 'event') group.events.push(record);
      else group.contexts.push(record);
      groups.set(key, group);
    }

    visibleEvents.forEach((event) => addRecord(event, 'event'));
    visibleContexts.forEach((context) => addRecord(context, 'context'));

    locationLayer.clearLayers();
    routeLayer.clearLayers();
    const visibleBounds = L.latLngBounds([]);
    groups.forEach((group) => {
      const eventIndexes = group.events.flatMap((event) => eventBranches.get(event.id) || []);
      const contextIndexes = group.contexts.flatMap((context) => contextBranches.get(context.id) || []);
      const indexes = [...new Set([...eventIndexes, ...contextIndexes])];
      const mode = group.events.length && group.contexts.length ? 'mixed' : group.contexts.length ? 'context' : 'event';
      const itemCount = group.events.length + group.contexts.length;
      const marker = L.marker(group.coordinates, {
        keyboard: true,
        riseOnHover: true,
        title: `${itemCount} mapped family item${itemCount === 1 ? '' : 's'} at ${group.name}`,
        alt: `Open mapped family information at ${group.name}`,
        icon: L.divIcon({
          className: 'genealogy-map-marker-host',
          html: pinHtml(itemCount, indexes, branches, mode),
          iconSize: mode === 'context' ? [42, 42] : [42, 50],
          iconAnchor: mode === 'context' ? [21, 21] : [21, 48],
          popupAnchor: mode === 'context' ? [0, -18] : [0, -45],
        }),
      }).bindPopup(popupHtml(group.name, group.events, group.contexts, peopleById, eventBranches, contextBranches, branches), { maxWidth: 450, maxHeight: 500 });
      marker.addTo(locationLayer);
      visibleBounds.extend(group.coordinates);
    });
    ...
    if (routeNote) routeNote.innerHTML = shownRoutes
      ...
      : '<strong>No movement line is being claimed</strong>Known locations are shown without connecting them unless an explicit movement route has been separately reviewed.';
    ...
    setStatus(`${visibleTotal} map item${visibleTotal === 1 ? '' : 's'} shown across ${groups.size} mapped location${groups.size === 1 ? '' : 's'}: ${visibleEvents.length} family event${visibleEvents.length === 1 ? '' : 's'}, ${visibleContexts.length} context item${visibleContexts.length === 1 ? '' : 's'}; ${unresolved.length} left unpinned.`);
  }
