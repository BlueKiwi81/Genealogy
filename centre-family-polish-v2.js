...
function setChildLabel(label, firstName, x) {
  const ns = 'http://www.w3.org/2000/svg';
  const key = `${firstName}|${x}`;
  if (label.dataset.centrePolishV2Key === key) return;
  label.dataset.centrePolishV2Key = key;
  label.textContent = '';
...
if (treeCanvas) {
  ...
  window.addEventListener('load', () => window.setTimeout(polishFamilyCentre, 120));
  window.setTimeout(polishFamilyCentre, 120);
}
