...
async function requestMagicLink(email, firstName) {
  localStorage.setItem('genealogyPendingName', firstName.trim());
...
async function ensureAccessRequest() {
  const fallback = state.session.user.email?.split('@')[0] || 'Family member';
  const displayName = localStorage.getItem('genealogyPendingName')?.trim() || fallback;
  const payload = {
...
function parentPairOf(personId) {
  ...
  if (fatherIndex >= 0) {
    slots[0] = candidates[fatherIndex];
    used.add(fatherIndex);
  }

  const motherIndex = candidates.findIndex((entry) => entry.person.gender === 'female');
  if (motherIndex >= 0) {
    slots[1] = candidates[motherIndex];
...
function siblingsOf(personId) {
  ...
  state.relationships.forEach((relationship) => {
    if (relationship.relationship_type === 'parent'
      ...
      directSiblingIds.add(relationship.person2_id);
    }
  });
...
function evidenceStyle(path, sourceStatus) {
  ...
  } else if (sourceStatus === 'family_supplied') {
    path.setAttribute('fill-opacity', '.82');
  } else {
    path.setAttribute('fill-opacity', '.92');
  }
}
...
function renderTree() {
  ...
  levels.forEach((entries, levelIndex) => {
    ...
    for (let slot = 0; slot < slots; slot += 1) {
      ...
      const [textX, textY] = polar(cx, cy, radiusMid, mid);
...
function renderPersonPanel(person) {
  ...
  partnersOf(person.id).forEach(({ person: partner, relationship }) => {
    const label = relationship.relationship_type === 'former_spouse'
      ? 'Former spouse'
  ...
  personDetails.innerHTML = lines
    .map(([label, value]) => `<div class="detail-line"><strong>${esc(label)}</strong>${esc(value)}</div>`)
    .join('');
}
...
function populateCentreSelect() {
  ...
  if (!state.centreId && state.people.length) {
    state.centreId = state.people.find((person) => person.slug === 'werner')?.id || state.people[0].id;
  }
...
async function enterApp() {
  ...
  try {
    ...
    if (!profile || profile.status !== 'approved') {
      const request = await ensureAccessRequest();
      treeCanvas.innerHTML = `<div class="empty-state"><h2>Welcome, ${esc(request.display_name)}</h2><p>Your secure sign-in worked. Your family access request is waiting for approval and for your account to be linked to the correct person in the tree.</p></div>`;
      setMessage(treeStatus, 'Signed in. Family access is awaiting approval.');
      return;
    }

    localStorage.removeItem('genealogyPendingName');
    ...
    treeCanvas.innerHTML = '<div class="empty-state"><h2>Unable to load the family archive</h2><p>The sign-in succeeded, but the family data could not be loaded. Please try again or contact the family editor.</p></div>';
    setMessage(treeStatus, error.message || 'Unable to load family data.', 'error');
  }
}
...
loginForm.addEventListener('submit', async (event) => {
  ...
  try {
    await requestMagicLink(email, firstName);
