  ...
  try {
    ...
    if (evidenceIds.length) {
      const { data: evidence, error: evidenceError } = await supabase
        .from('evidence_items')
        .select('id, title, original_filename, storage_path, review_status, evidence_type, source_class, contains_living_person_data, privacy_review_status, visibility')
        .in('id', evidenceIds);
    ...
    cards.forEach((card) => {
      ...
      const items = refs.map((ref) => evidenceById.get(ref.id) || ref);
      card.dataset.containsLivingData = items.some((item) => item.contains_living_person_data === true) ? 'true' : 'false';
      ...
      items.forEach((item) => {
        ...
        const privacy = item.contains_living_person_data === true
          ? 'Living-person information flagged - keep restricted'
          : `Privacy review: ${label(item.privacy_review_status || 'pending')}`;
        row.innerHTML = `<div class="evidence-review-meta"><span>${esc(name)}</span><small>${esc(label(item.evidence_type || item.source_class || 'source'))} | ${esc(privacy)}</small></div>`;
        ...
        open.addEventListener('click', (event) => {
          ...
          openEvidence(item.storage_path, name);
        });
        row.appendChild(open);
        box.appendChild(row);
      });
      const warning = document.createElement('p');
      warning.className = 'evidence-review-warning';
      warning.textContent = 'Approve the source only after opening it and checking that it belongs to the right person. Source approval does not by itself make every claim documented.';
...
async function reviewSourceCard(event) {
  ...
  const containsLivingData = card.dataset.containsLivingData === 'true';
  ...
  try {
    ...
    const { error: evidenceError } = await supabase
      ...
      .update({
        ...
        privacy_review_status: approved ? (containsLivingData ? 'restricted' : 'cleared') : 'restricted',
    ...
    const { error: contributionError } = await supabase
      ...
      .update({
        ...
        review_note: approved ? (editorText || 'Approved source record for evidence review.') : 'Rejected by family editor.',
        reviewed_by: session.user.id,
        reviewed_at: now,
      })
    ...
    setEditorMessage(approved
      ? `Record approved as a source. The original remains restricted${containsLivingData ? ' because living-person information was flagged' : ''}. Individual genealogical claims still require their own evidence-status decision.`
      : 'Record rejected. The original submission remains preserved and restricted.', 'success');
    refreshButton?.click();
  } catch (error) {
...
function installStyles() {
  ...
  style.textContent = `
    ...
    .evidence-review-row{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:7px 0;border-top:1px solid rgba(96,82,67,.12)}
    .evidence-review-row:first-of-type{border-top:0}
    .evidence-review-meta{display:grid;gap:2px;min-width:0}
    .evidence-review-meta span{font-size:.86rem;overflow-wrap:anywhere}
    .evidence-review-meta small{font-size:.72rem;line-height:1.35;color:#776c61;text-transform:none}
    .evidence-review-row .button{white-space:nowrap}
    .evidence-review-warning{margin:8px 0 0;padding:8px 9px;border-radius:8px;background:#fff3df;color:#6a5235;font-size:.76rem;line-height:1.4}
  `;
